...
??? info "一个没有讲到过的点"
    ...
    奈奎斯特定理（也称香农采样定理）规定，对于一个带宽有限的信号，为了能够无失真地从采样中恢复原始信号，采样频率 $f_s$ 必须**至少**是信号最高频率 $f_{max}$ 的两倍。
    ...
    因此，尽管题目中提到了这是一个 "3-kHz channel"（3000 Hz 信道），但由于它**受限于每1毫秒采样一次**（1000 Hz 采样率），该信道实际能有效利用的带宽（Bandwidth，即 $B$）只有 **500 Hz**。
