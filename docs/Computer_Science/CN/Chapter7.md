...
1. 用户 A 向 KDC 发送一个明文请求，内容包含 A 和 B 的身份标识（A, B），表示“我想和 B 进行安全通信”。
2. KDC 随机生成一个一次性会话密钥 $K_{AB}$。KDC 向 A 发回一个用 A 的主密钥 $K_A$ 加密的复合报文，里面包含：

- 会话密钥 $K_{AB}$。
- 给 B 的票据 (Ticket)：这个票据是用 B 的主密钥 $K_B$ 加密的，内容包含 A 的身份和会话密钥 $K_{AB}$。

3. A 把刚才从 KDC 拿到的那份“票据”发给 B。用户 B 使用自己的主密钥 $K_B$ 解开票据，从而安全地获取了 $K_{AB}$。此时，A 和 B 都拥有了相同的会话密钥 $K_{AB}$。
4. KDC可以在报文中加入时间戳，以防止报文的截获者利用之前已记录下的报文进行重放攻击。
