...
??? note "answer"
    ...
    建立第一个 TCP 连接需要 1 RTT,请求并接收 Web 页需要 1 RTT。浏览器支持并行 TCP 连接,因此在收到 Web 页后可同时建立 7 个并行的 TCP 连接，以请求和接收 7 个小图像文件。因此,总往返时间 RTT 数 = 1RTT(建立第一个 TCP 连接)+ 1RTT(请求 Web 页)+ 1RTT(建立7个并行的 TCP 连接) + 1RTT(请求7个小图像文件) = 4RTT。
...
??? note "answer"
    ...
    2. TCP 建立时第三次握手报文可以携带数据

    3. TCP 连接释放时第一次挥手报文可以携带数据

    4. 对于主动释放连接的一方，在收到对方的第二次挥手报文后，经过 2MSL 时间即可进入 CLOSED 状态，而不用考虑发出去的最后一个报文段是否被对方确认
