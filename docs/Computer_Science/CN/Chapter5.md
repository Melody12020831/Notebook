...
??? note "answer"
    ...
    UDP 首部中的“长度”字段记录了整个 UDP 用户数据报（首部 + 数据）的字节数。

    在传输过程中，除非发生分片，否则 UDP 层的数据内容和长度不会被改变。由于 IP 层负责处理分片和重组，UDP 报文作为一个整体封装在 IP 数据报中，其长度字段在发送端确定后，直到接收端都不会改变。

    II. UDP 检验和（保持不变）

    UDP 检验和用于验证 UDP 报文（及伪首部）在传输过程中是否出错。

    ”的校验。虽然计算时涉及 IP 地址（伪首部），但在不考虑 NAT 的前提下，源 IP 和目的 IP 保持不变，因此 UDP 检验和在传输过程中保持不变。

    Ⅲ. FCS 帧检验序列（一定改变）

    FCS 位于数据链路层（以太网帧）的尾部，用于校验当前这一跳的整个帧是否出错。

    由于每一跳的源 MAC、目的 MAC 以及 IP 首部中的 TTL 都会改变，导致每一跳生成的整个 MAC 帧内容都不同，因此每一跳必须重新计算并填充新的 FCS。

    IV. 目的 MAC 地址（一定改变）

    MAC 地址是物理层面的“下一跳”地址。

    在跨网络通信中，数据包每经过一个路由器，目的 MAC 地址就会从“当前网关的 MAC”变为“下一跳路由器的 MAC”，直到最后一跳变为“目标主机的 MAC”。

    V. 目的 IP 地址（保持不变）

    IP 地址是网络层面的“终点”地址。

    在不考虑 NAT 的情况下，目的 IP 地址指明了数据包的最终归宿。路由器通过查看该字段来查找路由表，但不会修改它。

    VI. IP 检验和（一定改变）

    IP 检验和仅覆盖 IP 首部。

    由于 IP 首部中的 TTL（生存时间） 字段在每经过一个路由器时都会减 1，首部内容发生了变化，因此路由器必须在每一跳都重新计算并更新 IP 检验和。
...
???+ example "question"
    TCP在 40Gb/s的线路上传送数据，若TCP充分利用了线路的带宽，则经过()的时间TCP会发生序号绕回(使用了之前用过的字节序号，已知$2^{32} / (40×10^9) = 0.859$)。
...
??? note "answer"
    ...
    A 向 B发送的第一个 SYN 段虽然不携带数据,但仍会消耗一个序号 211,A 向B发送的第 4 次挥手报文段的 seq 等于 985，说明之前发送的数据的序号为 212~983，因为在断开连接时发送的第一个 FIN 段会消耗一个序号，因此发送的总数据量为 983-212+1=772B。
...
!!! info
    如果所测量到的 $RTT$ 样本不正确，那么所计算出的 $RTT_S$ 和 $RTT_D$ 自然就不正确，进而所计算出的 $RTO$ 也就不正确。
...
设想出现这样的情况：报文段的时延突然增大很多，并且之后很长一段时间都会保持这种时延（这可能是因为网络拓扑发生了变化）。因此在原来得出的 $RTO$ 内，不会收到确认报文段，于是就重传报文段。但根据 Karn 算法，不考虑重传的报文段的 RTT 样本，因此 $RTO$ 就无法更新，这会导致报文段反复被重传。
