...
??? note "Answer"
    ...
    检验统计量 $\frac{\overline{X}-15000}{1500/\sqrt{n}} = 3.5$
    
    拒绝域 $W = \frac{\overline{X}-15000}{1500/\sqrt{n}} > Z_{0.05 = 1.645}$

    因为 $3.5 > 1.645$ ,所以拒绝 $H_0$ ,认为该厂生产的显像管寿命显著地高于规定的标准.
...
??? note "Answer"
    ...
    检验统计量 $T = \frac{\overline{X}-500}{6.5/\sqrt{n}} \approx -0.973$

    拒绝域 $W = |\frac{\overline{X}-500}{6.5/\sqrt{n}}| > t_{0.01}(9) = 2.8214$

    因为 $|-0.973| < 2.8214$ ,所以不能拒绝 $H_0$ ,认为机器工作正常.
...
??? note "Answer"
    ...
    $= P\{S^2 < 1.6664 | \sigma^2 = 4\} = P\{\frac{(16-1)S^2}{4} < 6.249 = \chi^2(15) \} \approx 0.024745305339540283$
