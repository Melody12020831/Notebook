...
定义：设随机试验的样本空间为$S = \{e\}$，若 $X=X(e)$ 为定义在样本空间$S$上的实值单值函数，则称 $X=X(e)$ 为随机变量。
...
则称 X 为服从**参数为 p 的 0−1分布**，也称为**两点分布**，并记为 $X \sim B(1,p)$ 或者 $X \sim 0−1(p)$
...
3. $F(x)$ 右连续，即 $F(x+0)=F(x)$ , $F(x) - F(x-0)=P(X = x)$ ;
4. $P(a< X \le b)=F(b)−F(a);$
5. 分布函数 $F(x)$ 在 $x = x_k$ 处有跳跃，其跳跃值为 $p_k = P(X=x_k)$ ;
6. 一般地，离散型随机变量的分布函数为阶梯函数。
7. 要注意是否取等的影响 $F(X \le x_1) = P(X = x_1)$ 而 $F(X < x_1) = 0$ ;
...
其中 $\lambda > 0$，则称 $X$ 服从**参数为** $\lambda$**的指数分布(exponential distribution)**，记为 $X \sim E(\lambda)$ 或 $X \sim Exp(\lambda)$
